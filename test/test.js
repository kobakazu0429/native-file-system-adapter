...
t("getFileHandle(create=false) returns existing files", async (root) => {
  existing_handle = await createFileWithContents(
    "existing-file",
    "1234567890",
    root
  );
...
t("WriteParams: truncate missing size param", async (root) => {
  handle = await createFileWithContents(
    "content.txt",
    "very long string",
    root
  );
...
t("write() fails when parent directory is removed", async (_root) => {
  // TODO: fix me
  // dir = await createDirectory('parent_dir', root)
...
t("truncate() fails when parent directory is removed", async (_root) => {
  // TODO: fix me
  // dir = await createDirectory('parent_dir', root)
...
t("Large real data test", async (_root) => {
  return;
